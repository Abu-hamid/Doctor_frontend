...
const Appointment = () => {
  ...
  const getAvailableSlots = async () => {
    ...
    if (!docInfo || !docInfo.slots_booked) {
      console.warn("Doctor info not available yet.");
      return;
    }
    let today = new Date();
    ...
    for (let i = 0; i < 7; i++) { // Next 7 days
      ...
      if (i === 0) {
        // For today, only show the next 8 hours
        endTime.setHours(currentDate.getHours() + 8, 0, 0, 0);
      } else {
        // For other days, show from 10 AM to 9 PM
      ...
      while (currentDate < endTime) {
        ...
        let dayOfWeek = daysOfWeek[currentDate.getDay()]; // Get day name
        ...
        if (isSlotAvailable) {
          timeSlots.push({
            ...
            time: formattedTime,
            day: dayOfWeek, // Add day name
            date: `${day}/${month}/${year}` // Add formatted date
          });
        }
